        ...
        <Box>
          {selectedId ? (
            ...
            <>
              ...
              <WatchedMovieList
                watched={watched}
                onDeleteWateched={handleDeleteWatched}
              />
            </>
          )}
        </Box>
  ...
  useEffect(
    function () {
      function callback(e) {
        ...
        if (e.code === "Enter") {
          // We need to read current property
...
function WatchedMovieList({ watched, onDeleteWateched }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie
          movie={movie}
          key={movie.imdbID}
          onDeleteWateched={onDeleteWateched}
        />
      ))}
    </ul>
  );
}
function WatchedMovie({ movie, onDeleteWateched }) {
  return (
    <li>
      <img src={movie.poster} alt={`${movie.title} poster`} />
      <h3>{movie.Title}</h3>
      <div>
        ...
        <button
          ...
          onClick={() => onDeleteWateched(movie.imdbID)}
        >
          X
        </button>
      </div>
